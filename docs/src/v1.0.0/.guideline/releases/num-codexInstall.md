---
title: NUM-CODEX DSF 0.9.0 Deployment
icon: install
---
This setup guide uses pre-build docker images for DSF Version 0.9.0. This guide is not suitable for HiGHmed organizations.  
If you are a member of HiGHmed, see [HiGHmed Install](highmedInstall.md).
...
Two Certificates from the DFN-PKI Global G2 (via DFN e.V.), GÉANT TCS (via DFN e.V.) or D-Trust (via TMF e.V.) are needed, more infos see [Authentication](docs/src/doc/guideline/generalInformation/authentication.md) TODO
...
4. Download and Extract Config Files  
    ...
    * **Test NUM-CODEX** instance:
        ...
        sudo tar --same-owner -zxvf dsf_codex_test_fhir_0_9_0.tar.gz
        ```
    * **Production NUM-CODEX** instance:
        ...
        sudo tar --same-owner -zxvf dsf_codex_prod_fhir_0_9_0.tar.gz
        ```
    _The `tar` command will unpack the config files at `/opt/fhir` assuming you changed into the `/opt` directory._

5. Verify that the `fhir` system user or group can write into the following folder
   * `/opt/fhir/log`

6. Add certificates and keys
...
7. Uncomment one of the certificate chain entries in the docker-compose file base on the certificate authority that signed your DSF FHIR server certificate (certificate A). For example use the following two lines if the server certificate is signed by `DFN-Verein Global Issuing CA`
    ```
    L102:  ssl_certificate_chain_file.pem:
    L103:    file: ./secrets/ssl_certificate_chain_file_DFN-Verein.pem
    ```

8. Modify database passwords
...
9. Modify the docker-compose.yml file and set environment variables to the appropriate values
...
2. Download and Extract Config Files  
    ...
    * **Test NUM-CODEX** instance:
        https://github.com/highmed/highmed-dsf/wiki/resources/dsf_codex_test_bpe_0_9_0.tar.gz
        ```
        cd /opt
        wget https://github.com/highmed/highmed-dsf/wiki/resources/dsf_codex_test_bpe_0_9_0.tar.gz
        sudo tar --same-owner -zxvf dsf_codex_test_bpe_0_9_0.tar.gz
        ```
    * **Production NUM-CODEX** instance:
        https://github.com/highmed/highmed-dsf/wiki/resources/dsf_codex_prod_bpe_0_9_0.tar.gz
        ```
        cd /opt
        wget https://github.com/highmed/highmed-dsf/wiki/resources/dsf_codex_prod_bpe_0_9_0.tar.gz
        sudo tar --same-owner -zxvf dsf_codex_prod_bpe_0_9_0.tar.gz
        ```
